...
$$y(x) = y_{base}(x) + \sum_{j=1}^d \beta_j \mathit{b}_j(x)$$
...
$$\mathbf{X}=\left[\begin{array}{ccc}| &  & |\\\mathbf{x}_{1} & \ldots & \mathbf{x}_{M}\\| &  & |\end{array}\right], \; \; \; \; \;  \; \; \mathbf{F} = \left[\begin{array}{ccc}-& \mathbf{f}_{1}^{T} & -\\ & \vdots\\- & \mathbf{f}_{N}^{T} & -\end{array}\right]$$
